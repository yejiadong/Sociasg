...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $userid = trim(mysqli_real_escape_string($link, $_POST['userid']));
    ...
    if (!empty($message) && !empty($userid) && !empty($groupid)) {
        // Prepare to pull data
        $query = "INSERT into Groupchats (chat_group_id, user_id, chat_msg) values ('" . $groupid . "', '" . $userid . "' , '" . $message . "')";
